...
### `GET /v1/datasets/{dataset_id}/cases`
* **Purpose**: List test cases for a dataset with pagination.
* **Query Parameters**:
  - `page` (optional, default: 1, min: 1)
  - `limit` (optional, default: 50, min: 1, max: 1000)
* **Output**: `list[TestCase]`

### `GET /v1/datasets/{dataset_id}/files`
* **Purpose**: List all files uploaded for a specific dataset.
* **Output**:
  ```json
  [
    {
...
### `GET /v1/evaluations`
* **Purpose**: List all offline batch evaluation jobs with pagination.
* **Query Parameters**:
  - `skip` (optional, default: 0, min: 0): Number of jobs to skip.
  - `limit` (optional, default: 50, min: 1, max: 1000): Number of jobs to return.
* **Output**: `list[BatchRunResult]`

### `GET /v1/evaluations/{evaluation_id}/summary`
* **Purpose**: Get aggregated metrics summary across all runs in an evaluation.
* **Output Schema (`BatchSummary`)**:
  ```json
  {
    "job_id": "9a38f321-4b12-4c59-bf12-45eef22495cc",
    "pipeline_id": "d1354bb6-3129-45be-bbff-a129efb07908",
...
### `GET /v1/evaluations/{evaluation_id}/pipelines`
* **Purpose**: Get all individual pipeline run results for the entire batch.
* **Output**: `list[PipelineRunResult]`

### `GET /v1/evaluations/{evaluation_id}/pipelines/{pipeline_run_id}`
* **Purpose**: Get a specific pipeline run result by its unique run ID.
* **Output**: `PipelineRunResult`

### `GET /v1/evaluations/{evaluation_id}/testcases/{testcase_id}`
* **Purpose**: Get the pipeline run result for a specific test case within the evaluation batch.
* **Output**: `PipelineRunResult`

### `GET /v1/evaluations/{evaluation_id}/metrics/{metric_id}`
* **Purpose**: Get all individual metric run results for a specific metric across the entire batch job.
* **Output**: `list[MetricRunResult]`
