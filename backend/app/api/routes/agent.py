...
class ChatRequest(BaseModel):
    ...
    metric_name: Optional[str] = None # If provided, load current yaml config to guide the agent

class ChatResponse(BaseModel):
    response_text: str | None
    updated_metric: Optional[MetricConfig] = None
...
def chat_with_agent(request: ChatRequest):
    ...
    if request.metric_name:
        try:
            ...
            current_yaml = yaml.dump(metric_config.model_dump(exclude_unset=True), sort_keys=False)
    ...
    try:
        result = agent_service.chat_with_agent(messages, current_yaml_config=current_yaml)
    ...
    if called_tool_args:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to parse or save metric config from agent: {str(e)}")
